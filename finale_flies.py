...
random and buy-and-hold, over the whole locked test set. Once under the real rules (5 bps a
side, a 3-bar minimum hold, fills at the next bar's open), then again with the fees set to
zero so the cost of trading can be shown. The minimum hold stays on in both: it is a rule,
not a fee.
...
from finale_shared import (PASSES, TRIBES, build_agents, calibration_charts, champions_of, check_run,
                           common_args, describe, last_generation, part_path, require_gpu,
                           trade_champions, window_of, write_part)
